...
// network still land their shards: timed-out writes must settle the limiter
// rather than spend every host's attempts and empty the pool.
func TestUploadSaturatedNetwork(t *testing.T) {
	...
	// the host timeout is short enough that probing past what the network
	// carries times writes out
